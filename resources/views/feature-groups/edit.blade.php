<x-layouts.user title='機能グループ編集'>
    ...
    <section class='p-4'>
        <form action="{{ route('feature-groups.update', $featureGroup->id) }}" method="post">
            @csrf
            @method('PUT')
            <x-text-field label="name" name='name' :value="old('name', $featureGroup->name)" :error="$errors->first('name')" />
            <x-text-field multiline label="memo" name='memo' :value="old('memo', $featureGroup->memo)" :error="$errors->first('memo')" />
